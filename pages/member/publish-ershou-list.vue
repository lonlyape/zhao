<template>
	<view class="container">
		<list-top defaultActive="ershou"></list-top>
		<view class="list-wrap">
			<view class="list-item" v-for="(item,index) in lists" :key="index">
				<view class="t">
					<text class="title" @click="jump('/pages/ershou/details?ershou_id='+item.id)">{{item.title}}</text>
				...
				<view class="b">
					...
					<view class="btn-group">
						...
						<block v-else-if="item.status != 3">
							<text class="btn" @click="jump('modify-ershou?ershou_id='+item.id)">修改</text>
						</block>
	...
	export default{
		...
		methods:{
			async getData(){
				...
				const data = {user_id:this.userInfo.uid,page:(this.refreshing ? 1 : this.page)};
				...
				await this.request.post(this.api.getUserUsedList,{
					data:data
				}).then(res => {
					if(res.code){
						if (this.refreshing) {
			...
			setting(item){
				const _this = this;
				const data = {used_id:item.id,user_id:this.userInfo.uid,status:3};
				uni.showModal({
					title:'提示',
					content:'确定要'+this.settingName+'吗？',
					success:(e=>{
						if(e.confirm){
							this.request.post(this.api.updateUsedStatus,{
								data:data
							}).then(res=>{
								if(res.code){
									dispose();
			...
			deletes(item,index){
				...
				uni.showModal({
					...
					success:(e=>{
						if(e.confirm){
							this.request.post(this.api.deleteUsedInfo,{
								data:{used_id:id,user_id:this.userInfo.uid}
							}).then(res=>{
								if(res.code){
